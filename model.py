...
*** Optimized DeepACE: DeepACE_wmse, by Tom Gajecki & Yichi Zhang ***
*** weighted MSE (mean squared error) instead of MSE as loss function ***
...
class Decoder(tf.keras.layers.Layer):
    def __init__(self, n_electrodes, N, **kwargs):
...
class DeepACE():
    def __init__(self, args):
        ...
        self.decoder = Decoder(self.n_electrodes, self.N, name="Decoder")
...
class WMSE(tf.keras.losses.Loss):
    def __init__(self, **kwargs):
        super(WMSE, self).__init__(**kwargs)

    def call(self, y_true, y_pred):
        x = tf.square(y_true - y_pred)
        y = tf.multiply(tf.square(y_true - y_pred), 10)  # weight=10 for bands should not be selected
        result = tf.where(y_true > 0, x, y)
