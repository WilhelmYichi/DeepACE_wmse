...
from model import DeepACE, WMSE
...
def train(args):
    ...
    tf.keras.utils.plot_model(model, to_file=os.path.join(model_dir, 'model.pdf'), show_layer_names=False)  # 画网络结构图
...
def test(args, model):
    ...
    save_path = os.path.join(model_dir, "LGF_sub_HSMm_CCITT")
